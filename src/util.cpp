...
const char * const BITCOIN_CONF_FILENAME = "taurus.conf";
const char * const BITCOIN_PID_FILENAME = "taurusd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "taurus" is a composite category enabling all Taurus-related debug output
            if(ptrCategory->count(string("taurus"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\TaurusCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\TaurusCore
    // Mac: ~/Library/Application Support/TaurusCore
    // Unix: ~/.tauruscore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "TaurusCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/TaurusCore";
#else
    // Unix
    return pathRet / ".tauruscore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty taurus.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override taurus.conf
