...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Taurus database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Taurus datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Taurus datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Taurus 0.7 datadir."
    ;;
  3)
    echo "Detected Taurus pre-0.8 datadir."
    ;;
  4)
    echo "Detected Taurus 0.8 datadir."
    ;;
esac
